...
- AC1 (AC-rules-and-decision-01): Given an authorized persona creates or changes a decision table, when the request is submitted, then Rules And Decision validates mandatory data, owner, lifecycle state, tenant/geography boundary, policy, source authority, and dependency references before accepting the work.
- AC2 (AC-rules-and-decision-02): Given a decision table depends on source records mastered by another app, when the dependency is evaluated, then Rules And Decision records the master app, source identifier, correlation ID, freshness timestamp, and confidence level rather than copying mutable source data.
- AC3 (AC-rules-and-decision-03): Given eligibility outcome is required, when the persona approves, rejects, or escalates the decision, then Rules And Decision captures approver, reason code, effective date, expiry, before/after state, and evidence links.
- AC4 (AC-rules-and-decision-04): Given rule authoring changes state, when downstream consumers must react, then Rules And Decision publishes a versioned event with changed fields, idempotency key, actor, source channel, tenant, and correlation ID.
- AC5 (AC-rules-and-decision-05): Given validation fails for rule set, when the failure is correctable, then Rules And Decision opens an exception task with severity, owner, due date, blocked dependency, retry path, and compensating control where needed.
- AC6 (AC-rules-and-decision-06): Given an operator searches decision service, when the record is opened, then Rules And Decision shows lifecycle state, related entities, lineage or trace, policy decisions, comments, approvals, evidence, SLA/OLA timers, and allowed next actions.
- AC7 (AC-rules-and-decision-07): Given closure is requested, when any downstream handoff, reconciliation, evidence snapshot, or audit requirement is incomplete, then Rules And Decision blocks closure or records an approved exception with expiry and accountable owner.
- AC8 (AC-rules-and-decision-08): Given reporting, audit, or release evidence is requested, when the report is generated, then Rules And Decision exposes metrics for volume, aging, failures, policy overrides, automation rate, data quality or conformance, and completion quality without direct database access.
