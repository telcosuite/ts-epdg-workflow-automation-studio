...
# Cross-App Journey Templates And Saga Compensation Feature Specification
...
Feature area slug: `cross-app-journey-templates-and-saga-compensation`

## Feature Intent

Provide prebuilt cross-app journey templates, saga orchestration, durable state, compensation, resumable execution, exception repair, and failure recovery for long-running telecom workflows.
...
| --- | --- |
| journey template | Create, version, validate, approve, operate, evidence, retire, and reconcile the journey template within the Workflow And Automation Studio boundary. |
| saga instance | Create, version, validate, approve, operate, evidence, retire, and reconcile the saga instance within the Workflow And Automation Studio boundary. |
| compensation step | Create, version, validate, approve, operate, evidence, retire, and reconcile the compensation step within the Workflow And Automation Studio boundary. |
| resumable checkpoint | Create, version, validate, approve, operate, evidence, retire, and reconcile the resumable checkpoint within the Workflow And Automation Studio boundary. |
| dependency graph | Create, version, validate, approve, operate, evidence, retire, and reconcile the dependency graph within the Workflow And Automation Studio boundary. |
| repair task | Create, version, validate, approve, operate, evidence, retire, and reconcile the repair task within the Workflow And Automation Studio boundary. |
...
| --- | --- |
| template approval | Assigned owner approves, rejects, expires, or escalates template approval with reason, evidence, SoD check, and audit trail. |
| compensation trigger | Assigned owner approves, rejects, expires, or escalates compensation trigger with reason, evidence, SoD check, and audit trail. |
| resume versus rollback | Assigned owner approves, rejects, expires, or escalates resume versus rollback with reason, evidence, SoD check, and audit trail. |
| dependency readiness | Assigned owner approves, rejects, expires, or escalates dependency readiness with reason, evidence, SoD check, and audit trail. |
| closure acceptance | Assigned owner approves, rejects, expires, or escalates closure acceptance with reason, evidence, SoD check, and audit trail. |

## Personas, Jobs, And Outcomes

| Persona | Job to be done | Outcome |
| --- | --- | --- |
| Process owner | models lifecycle states, transitions, timers, assignments, approvals, exception paths, and journey templates. | Uses journey template lifecycle, template approval evidence, and exception queues to complete the job without direct database access or shadow spreadsheets. |
| Journey owner | owns E2E journey KPIs, saga compensation, bottleneck remediation, and experience closure. | Uses journey template lifecycle, template approval evidence, and exception queues to complete the job without direct database access or shadow spreadsheets. |
| Automation architect | designs playbooks, action adapters, preconditions, rollback, safety, and emergency break-glass controls. | Uses journey template lifecycle, template approval evidence, and exception queues to complete the job without direct database access or shadow spreadsheets. |
| Operations manager | manages work queues, priorities, SLAs, escalations, manual interventions, and productivity. | Uses journey template lifecycle, template approval evidence, and exception queues to complete the job without direct database access or shadow spreadsheets. |
| Rule owner | governs decision tables, eligibility, routing, risk, versioning, and simulation evidence. | Uses journey template lifecycle, template approval evidence, and exception queues to complete the job without direct database access or shadow spreadsheets. |
| QA / release manager | validates workflow, rule, configuration, and automation changes before production release. | Uses journey template lifecycle, template approval evidence, and exception queues to complete the job without direct database access or shadow spreadsheets. |
| Platform admin | governs configuration packages, extensions, tenant variants, permissions, and rollback. | Uses journey template lifecycle, template approval evidence, and exception queues to complete the job without direct database access or shadow spreadsheets. |

## Core Workflows

| Workflow | Trigger | Validation and decision | Orchestration and handoff | Exception path | Completion evidence |
| --- | --- | --- | --- | --- | --- |
| Journey template design | journey template design is requested by UI, API, event, schedule, release gate, or control owner. | Validate owner, lifecycle state, tenant/geography boundary, source authority, template approval, and required evidence. | Orchestrate journey template state, publish status events, and hand off tasks to the owning BSS, OSS, platform, security, data, test, or workflow app. | Route missing dependency, failed policy, stale version, or downstream outage to an accountable queue with retry, rollback, compensation, or approval path. | Close only when journey template state, audit trail, evidence links, metrics, and downstream acknowledgements are complete. |
| Saga execution | saga execution is requested by UI, API, event, schedule, release gate, or control owner. | Validate owner, lifecycle state, tenant/geography boundary, source authority, template approval, and required evidence. | Orchestrate journey template state, publish status events, and hand off tasks to the owning BSS, OSS, platform, security, data, test, or workflow app. | Route missing dependency, failed policy, stale version, or downstream outage to an accountable queue with retry, rollback, compensation, or approval path. | Close only when journey template state, audit trail, evidence links, metrics, and downstream acknowledgements are complete. |
| Compensation handling | compensation handling is requested by UI, API, event, schedule, release gate, or control owner. | Validate owner, lifecycle state, tenant/geography boundary, source authority, template approval, and required evidence. | Orchestrate journey template state, publish status events, and hand off tasks to the owning BSS, OSS, platform, security, data, test, or workflow app. | Route missing dependency, failed policy, stale version, or downstream outage to an accountable queue with retry, rollback, compensation, or approval path. | Close only when journey template state, audit trail, evidence links, metrics, and downstream acknowledgements are complete. |
| Exception repair and closure | exception repair and closure is requested by UI, API, event, schedule, release gate, or control owner. | Validate owner, lifecycle state, tenant/geography boundary, source authority, template approval, and required evidence. | Orchestrate journey template state, publish status events, and hand off tasks to the owning BSS, OSS, platform, security, data, test, or workflow app. | Route missing dependency, failed policy, stale version, or downstream outage to an accountable queue with retry, rollback, compensation, or approval path. | Close only when journey template state, audit trail, evidence links, metrics, and downstream acknowledgements are complete. |

## Edge Cases

- Macd order changes billing but activation compensation fails: keep source authority, policy decision, exception owner, and downstream handoff visible to the accountable persona.
- Partner onboarding saga waits on certification evidence: keep source authority, policy decision, exception owner, and downstream handoff visible to the accountable persona.
- Build-to-launch template blocks commercial readiness after failed synthetic test: keep source authority, policy decision, exception owner, and downstream handoff visible to the accountable persona.

## Acceptance Criteria

1. **AC-cross-app-journey-templates-and-saga-compensation-01:** Given an authorized persona creates or changes a journey template, when the request is submitted, then Cross-App Journey Templates And Saga Compensation validates mandatory data, owner, lifecycle state, tenant/geography boundary, policy, source authority, and dependency references before accepting the work.
2. **AC-cross-app-journey-templates-and-saga-compensation-02:** Given a journey template depends on source records mastered by another app, when the dependency is evaluated, then Cross-App Journey Templates And Saga Compensation records the master app, source identifier, correlation ID, freshness timestamp, and confidence level rather than copying mutable source data.
3. **AC-cross-app-journey-templates-and-saga-compensation-03:** Given template approval is required, when the persona approves, rejects, or escalates the decision, then Cross-App Journey Templates And Saga Compensation captures approver, reason code, effective date, expiry, before/after state, and evidence links.
4. **AC-cross-app-journey-templates-and-saga-compensation-04:** Given journey template design changes state, when downstream consumers must react, then Cross-App Journey Templates And Saga Compensation publishes a versioned event with changed fields, idempotency key, actor, source channel, tenant, and correlation ID.
5. **AC-cross-app-journey-templates-and-saga-compensation-05:** Given validation fails for saga instance, when the failure is correctable, then Cross-App Journey Templates And Saga Compensation opens an exception task with severity, owner, due date, blocked dependency, retry path, and compensating control where needed.
6. **AC-cross-app-journey-templates-and-saga-compensation-06:** Given an operator searches compensation step, when the record is opened, then Cross-App Journey Templates And Saga Compensation shows lifecycle state, related entities, lineage or trace, policy decisions, comments, approvals, evidence, SLA/OLA timers, and allowed next actions.
7. **AC-cross-app-journey-templates-and-saga-compensation-07:** Given closure is requested, when any downstream handoff, reconciliation, evidence snapshot, or audit requirement is incomplete, then Cross-App Journey Templates And Saga Compensation blocks closure or records an approved exception with expiry and accountable owner.
8. **AC-cross-app-journey-templates-and-saga-compensation-08:** Given reporting, audit, or release evidence is requested, when the report is generated, then Cross-App Journey Templates And Saga Compensation exposes metrics for volume, aging, failures, policy overrides, automation rate, data quality or conformance, and completion quality without direct database access.

## Negative Scenarios

| Scenario | Expected behavior |
| --- | --- |
| Unauthorized actor attempts to view or mutate journey template | deny access, mask sensitive context, and record the policy decision. |
| Cross-tenant or cross-residency request references journey template | block the transaction unless an approved exception and transfer control exist. |
| Duplicate, stale, late, or out-of-order event changes journey template | apply idempotency, source priority, version checks, and replay controls. |
...
| Manual override is requested for journey template | require reason, approval, expiry, evidence, SoD check, and post-action review. |
| Retention, legal hold, consent, export control, or privacy policy conflicts with journey template | stop deletion, export, masking, or disclosure until the legal or privacy decision is recorded. |
| Bulk or project-scale update touches many journey template records | provide validation preview, partial failure report, rollback strategy, and operator approval before commit. |
| High-volume operational period stresses journey template processing | preserve critical transactions with back-pressure, pagination, async export, queue aging alerts, and runbook escalation. |
...
- Uses TMF701 Process Flow and references domain TMF APIs such as TMF622 Product Order, TMF641 Service Order, TMF652 Resource Order, TMF621 Trouble Ticket, and TMF655 Change Management as journey participants.
- JourneyTemplate, SagaInstance, CompensationStep, ResumableCheckpoint, and JourneyRepairTask extension APIs are required.
...
- Dashboards must show journey template volume, state distribution, queue aging, failures, retries, manual overrides, policy rejections, downstream latency, and completion quality.
- Alerts must trigger for stuck journey template workflows, integration outages, event publication failures, evidence gaps, unusual access patterns, SLA/OLA breach risk, and reconciliation mismatches.
...
This refinement converts the feature review material for Cross-App Journey Templates And Saga Compensation into delivery slices that can become epics, stories, API contracts, migrations, and test cases. Treat Workflow And Automation Studio as the owning application for this feature within Suite Enterprise Platform, Data, And Governance and schema `workflow_automation`.

| Workstream | Build-ready delivery guidance |
| --- | --- |
| UX and workflow | Build the Cross-App Journey Templates And Saga Compensation workbench for Process owner, Journey owner, Automation architect, Operations manager, Rule owner, QA / release manager. Include search or intake, guided validation, detail view, lifecycle timeline, decision panel, evidence drawer, exception queue, bulk or replay controls where relevant, saved filters, SLA/OLA aging, empty/error states, and role-aware masking. The UI must expose approve, reject, promote, correct, operate, retire, and reconcile journey template and block closure when required evidence, approval, reconciliation, or downstream acknowledgement is missing. |
| API and events | Implement command and query APIs around cross-app-journey-templates-and-saga-compensation using TMF701, TMF622, TMF641, TMF652, TMF621, TMF655. Command APIs must cover create, update, lifecycle transition, assign, approve, reject, cancel, retry, correct, export, and close where the feature lifecycle uses those actions. Query APIs must cover search, detail, timeline, related entities, work queues, metrics, audit retrieval, evidence retrieval, and dependency status. Events must cover created, updated, stateChanged, exceptionRaised, exceptionResolved, approved, rejected, cancelled, completed, corrected, and evidenceCaptured states where relevant. JourneyTemplate, SagaInstance, CompensationStep, ResumableCheckpoint, and JourneyRepairTask extension APIs are required. Every command, query, and event must carry tenant/brand/market where applicable, actor, source channel, reason code, idempotency key, correlation ID, external reference, lifecycle state, and version metadata. |
| Data and controls | Persist journey template, saga instance, compensation step inside `workflow_automation` with typed lifecycle, owner, status reason, timestamps, policy decision, source freshness, confidence, old/new value, evidence, and reconciliation fields. Workflow And Automation Studio owns the app-local lifecycle and evidence records for Cross-App Journey Templates And Saga Compensation; consumers must use APIs, events, projections, workflow tasks, or certified data products. Keep TMF payloads, extension characteristics, imported evidence, and low-stability metadata in JSONB while promoting operationally searched lifecycle fields to typed columns. |
| Integration and handoff | Exchange saga instance, compensation step, resumable checkpoint, dependency graph with BSS/OSS domain APIs, TMF701 process flow runtime, event mesh, rules engine, intent engine only through APIs, events, workflow tasks, governed projections, adapters, evidence packages, or certified data products. Show source owner, freshness, confidence, dependency state, retry status, blocked consumer, and completion evidence so the app does not create shadow mastership or direct cross-schema coupling. |
| Security, privacy, and compliance | Enforce RBAC/ABAC, tenant and residency boundaries, least privilege, separation of duties, masking, purpose limitation, retention, legal hold, export control, manual override expiry, immutable audit, and evidence chain of custody for Cross-App Journey Templates And Saga Compensation. Sensitive customer, revenue, partner, security, network, credential, or regulatory evidence must be masked unless the persona has explicit operational purpose. |
| Tests and operations | Create unit, API contract, event replay/idempotency, workflow, integration, migration, data reconciliation, security/privacy, accessibility/localization, performance, dashboard, alert, and runbook tests for Cross-App Journey Templates And Saga Compensation. Cover happy path, assisted path, automated path, exception path, bulk/project path, stale or duplicate input, downstream outage, policy denial, manual override, and reconciliation mismatch. Use the existing review scope - saga operations, compensation handling, rule blast-radius simulation, action adapter governance, tenant-safe configuration promotion, and process mining feedback. - as mandatory backlog and test evidence. |

Implementation notes:

- Treat Workflow And Automation Studio as the lifecycle owner for journey template, saga instance, compensation step; referenced data such as saga instance, compensation step, resumable checkpoint, dependency graph must remain references, snapshots, projections, evidence packages, or consumer acknowledgements unless the source file explicitly gives this app mastership.
...
- Product owner has accepted journey template lifecycle behavior, personas, journeys, negative scenarios, and operational evidence.
...
- Operations/SRE owner has dashboards, alerts, runbooks, error budgets, replay/retry procedures, and support handoff for journey template.
...
- AC1 (AC-cross-app-journey-templates-and-saga-compensation-01): Given an authorized persona creates or changes a journey template, when the request is submitted, then Cross-App Journey Templates And Saga Compensation validates mandatory data, owner, lifecycle state, tenant/geography boundary, policy, source authority, and dependency references before accepting the work.
- AC2 (AC-cross-app-journey-templates-and-saga-compensation-02): Given a journey template depends on source records mastered by another app, when the dependency is evaluated, then Cross-App Journey Templates And Saga Compensation records the master app, source identifier, correlation ID, freshness timestamp, and confidence level rather than copying mutable source data.
- AC3 (AC-cross-app-journey-templates-and-saga-compensation-03): Given template approval is required, when the persona approves, rejects, or escalates the decision, then Cross-App Journey Templates And Saga Compensation captures approver, reason code, effective date, expiry, before/after state, and evidence links.
- AC4 (AC-cross-app-journey-templates-and-saga-compensation-04): Given journey template design changes state, when downstream consumers must react, then Cross-App Journey Templates And Saga Compensation publishes a versioned event with changed fields, idempotency key, actor, source channel, tenant, and correlation ID.
- AC5 (AC-cross-app-journey-templates-and-saga-compensation-05): Given validation fails for saga instance, when the failure is correctable, then Cross-App Journey Templates And Saga Compensation opens an exception task with severity, owner, due date, blocked dependency, retry path, and compensating control where needed.
- AC6 (AC-cross-app-journey-templates-and-saga-compensation-06): Given an operator searches compensation step, when the record is opened, then Cross-App Journey Templates And Saga Compensation shows lifecycle state, related entities, lineage or trace, policy decisions, comments, approvals, evidence, SLA/OLA timers, and allowed next actions.
- AC7 (AC-cross-app-journey-templates-and-saga-compensation-07): Given closure is requested, when any downstream handoff, reconciliation, evidence snapshot, or audit requirement is incomplete, then Cross-App Journey Templates And Saga Compensation blocks closure or records an approved exception with expiry and accountable owner.
- AC8 (AC-cross-app-journey-templates-and-saga-compensation-08): Given reporting, audit, or release evidence is requested, when the report is generated, then Cross-App Journey Templates And Saga Compensation exposes metrics for volume, aging, failures, policy overrides, automation rate, data quality or conformance, and completion quality without direct database access.
